...
def _get_printer():
    """Open the ESC/POS printer, detaching usblp atomically via libusb.

    Monkey-patching usb.core.find injects set_auto_detach_kernel_driver(True)
    onto the device handle before escpos calls set_configuration().  With that
    flag set, libusb detaches any kernel driver (usblp) as part of
    set_configuration() — atomically, with no race window.  No modprobe/sudo
    needed.
    """
    ...
    def _find_with_auto_detach(*args, **kwargs):
        dev = _orig_find(*args, **kwargs)
        if dev is not None:
            try:
                dev.set_auto_detach_kernel_driver(True)
            except Exception as exc:
                logger.debug("set_auto_detach_kernel_driver: %s", exc)
        return dev

    usb.core.find = _find_with_auto_detach
    try:
